    ...
    <div className='p-2 m-2 w-72'>
      ...
      <ul className='flex justify-start items-start'>
        ...
        <div>
          ...
          <li className='text-gray-500 text-[13px]'>{videoInfo?.statistics?.viewCount ? formatCompactNumber(videoInfo?.statistics?.viewCount) : 0} views  {(Math.abs(new Date(publishedAt) - new Date()) / (60 * 60 * 24 * 1000)).toFixed(1)} days ago</li>
        </div>
      </ul>
    </div>
  )
